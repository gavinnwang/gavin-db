...
TableHeap::TableHeap(BufferPoolManager *bpm, page_id_t table_info_page_id)
	...
	ASSERT(table_info_page->GetTableInfo().name_.size() > 0, "table name is empty");
	...
	ASSERT(table_info_page->GetLastTablePageId() != INVALID_PAGE_ID &&
	           table_info_page->GetFirstTablePageId() != INVALID_PAGE_ID && table_info_page->GetLastTablePageId() > 0 &&
...
auto TableHeap::InsertTuple(const TupleMeta &meta, const Tuple &tuple) -> std::optional<RID> {
	...
	auto table_info_pg = bpm_->FetchPageWrite(table_info_page_id_);
	auto table_info_page = table_info_pg.AsMut<TableInfoPage>();
	auto page_guard = bpm_->FetchPageWrite(table_info_page->GetLastTablePageId());
	while (true) {
