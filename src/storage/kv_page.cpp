...
auto KVPage::Put(const std::string &key, const std::string &value) -> bool {
  ...
  if (key.size() > std::numeric_limits<uint16_t>::max()) {
    std::cout << "not allowed to have a key with size greater than max uint16"
              << std::endl;
    return false;
  }
  std::string temp_val;
  ...
  kv_info_[num_keys_] =
      std::make_tuple(*offset, key.size(), value.size(), meta);
  num_keys_++;
...
auto KVPage::GetNextTupleOffset(const std::string &key,
...
void KVPage::Compact() {
  ...
  for (uint16_t cur_i = 0; cur_i < num_keys_ + deleted_keys; cur_i++) {
    ...
    memcpy(page_start_ + last_offset - kv_size, page_start_ + cur_offset,
           kv_size);
