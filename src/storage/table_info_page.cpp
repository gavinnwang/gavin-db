...
#include <cstring>
namespace db {
void TableInfoPage::Init(const std::string name, const Schema &schema, const table_oid_t table_oid) {
...
auto TableInfoPage::GetTableInfo() const -> TableInfo {
	ASSERT(table_info_offset_ >= TABLE_INFO_PAGE_HEADER_SIZE, "table info offset not initialized correctly");
	ASSERT(table_info_offset_ < PAGE_SIZE, "table info offset not initialized correctly");
...
void TableInfoPage::UpdateTableSchema(const Schema &schema) {
	...
	auto new_table_info = TableInfo {schema, old_table_info.name_, old_table_info.table_oid_};
...
void TableInfoPage::StoreTableInfo(const TableInfo &table_info) {
	...
	if (table_info_offset_ < TABLE_INFO_PAGE_HEADER_SIZE) {
		throw Exception("Schema + table name is too large to be stored on one page");
	}
