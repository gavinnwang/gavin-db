...
public:
  ...
  template <class T>
  void WritePropertyWithDefault(const field_id_t field_id, const char *tag,
...
protected:
  // Unique Pointer Ref
  template <typename T> void WriteValue(const std::unique_ptr<T> &ptr) {
    WriteValue(ptr.get());
  }

  // Shared Pointer Ref
  template <typename T> void WriteValue(const std::shared_ptr<T> &ptr) {
    WriteValue(ptr.get());
  }

  // Pointer
  template <typename T> void WriteValue(const T *ptr) {
    if (ptr == nullptr) {
  ...
  template <class K, class V> void WriteValue(const std::pair<K, V> &pair) {
    // OnObjectBegin();
  ...
  // Vector
  template <class T> void WriteValue(const std::vector<T> &vec) {
    auto count = vec.size();
    OnListBegin(count);
    for (auto &item : vec) {
      WriteValue(item);
...
protected:
  ...
  virtual void OnOptionalPropertyBegin(const field_id_t field_id,
                                       const char *tag, bool present) = 0;
